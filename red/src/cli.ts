...
import { doksWorkflow } from "./workflow.ts";

export const lifecycleCommands = ["build", "create", "check", "kubeconfig", "registry", "delete"];

export const usage = "Usage: red <build|create|check|kubeconfig|registry|delete> [-f|--file colors.yml] [--dry-run]";
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(doksWorkflow, withFile, { allowedEvents: lifecycleCommands });
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(doksWorkflow, defaultArgs(args), { allowedEvents: lifecycleCommands });
  }
