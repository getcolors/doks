...
from .workflow import doks_workflow

USAGE = "Usage: blue <build|create|check|kubeconfig|registry|delete> [-f|--file colors.yml] [--dry-run]"

LIFECYCLE = ("build", "create", "check", "kubeconfig", "registry", "delete")
...
async def run(*args):
    ...
    if command in LIFECYCLE:
        return await run_cli(doks_workflow, args, allowed_events=list(LIFECYCLE))
